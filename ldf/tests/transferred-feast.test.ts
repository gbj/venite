...
const findLiturgicalDay = (d: Date) =>
    Promise.resolve(LITURGICAL_DAYS[`${d.getFullYear()}/${d.getMonth() + 1}/${d.getDate()}`]),
  findSpecialDay = (slug: string) => Promise.resolve(SPECIAL_DAYS.find((day) => day.slug == slug)),
  findFeastDay = (d: Date) =>
    Promise.resolve(FEAST_DAYS.find((day) => day.mmdd == `${d.getMonth() + 1}/${d.getDate()}`));

describe('transferredFeast', () => {
  it('is undefined if there are no feasts', async () => {
    expect(
      await transferredFeast(findLiturgicalDay, findSpecialDay, findFeastDay, dateFromYMD('2020', '6', '3')),
    ).toBeUndefined();
  });

  it('does not transfer observed red-letter days forward', async () => {
    expect(
      await transferredFeast(findLiturgicalDay, findSpecialDay, findFeastDay, dateFromYMD('2020', '8', '7')),
    ).toBeUndefined();
  });

  it('transfers Visitation to Monday when it overlaps with Pentecost the day before', async () => {
    expect(
      (await transferredFeast(findLiturgicalDay, findSpecialDay, findFeastDay, dateFromYMD('2020', '6', '1')))?.slug,
    ).toEqual('the-visitation');
  });
  ...
  it('does not transfer Monday feasts observed on Monday as if Sunday had blocked them', async () => {
    expect(
      (await transferredFeast(findLiturgicalDay, findSpecialDay, findFeastDay, dateFromYMD('2020', '8', '25')))?.slug,
    ).toEqual(undefined);
  });
});
