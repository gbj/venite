...
export function findCollect(collects : LiturgicalDocument[], day : LiturgicalDay, sundayFirst : boolean = true) : LiturgicalDocument | null {
  const observedDay = day.propers || day.slug,
    ...
    blackLetterCollects = blackLetterDays.map(holyday =>
      (holyday.category || []).map(category =>
        collects.filter(collect => collect.slug === category)
  ...
  if(redLetterOrSunday || blackLetterCollects.length > 0) {
    if(sundayFirst) {
      const collects = new Array(redLetterCollect || sundayCollect || new LiturgicalDocument())
        .concat(blackLetterCollects);
