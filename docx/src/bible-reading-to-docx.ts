...
export async function bibleReadingToDocx(
  doc: BibleReading,
  ...
  if (doc.style === "long") {
    const p = paragraphs(doc)
        .map((p) => paragraph(p, displaySettings, localeStrings))
        .flat(),
      heading = headingLine(doc),
      intro = await introLine(doc, displaySettings, localeStrings);

    return [heading, ...intro, ...p].filter(notEmpty);
  } else {
      ...
      p =
        includeResponse && shortResponse
          ? ps
              ...
              .flat()
          : ps
              .map((p) => paragraph(p, displaySettings, localeStrings, false))
              .flat();
    ...
    return [
      ...
      includeResponse && !shortResponse
        ? new Paragraph({ children: [new TextRun(" "), responseNode] })
        : null,
    ].filter(notEmpty);
  }
}
...
async function introLine(
  ...
  if (doc?.metadata?.intro) {
    ...
    if (doc.metadata.compiled_intro) {
      return await docxChildrenFromLDF(
