...
export async function ldfToDocx(
  ...
  const styles = stylesFromLDF(inDoc),
    ...
    children = await docxChildrenFromLDF(
      inDoc,
      displaySettings,
      localeStrings,
      outDoc
...
export async function docxChildrenFromLDF(
  inDoc: LiturgicalDocument,
  displaySettings: DisplaySettings,
  ...
  switch (inDoc?.type) {
    case "liturgy":
      return (
        await Promise.all(
          ((inDoc as Liturgy).value || []).map(
            async (subDoc: LiturgicalDocument) =>
              await docxChildrenFromLDF(
                subDoc,
                displaySettings,
                localeStrings,
                docxDoc
