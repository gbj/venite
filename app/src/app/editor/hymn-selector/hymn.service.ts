...
const hymnNumberFromHymn = (hymn : Hymn) => {
  const constainsS = hymn.number.includes('S'),
    ...
    adjustedNumber = hymn.source.includes('LEV') ? number + 2000 : number + 1000;
...
export class HymnService {
  ...
  search(search : string) : Observable<Hymn[]> {
    return this.afs.collection<Hymn>('Hymns').valueChanges().pipe(
      map((hymns : Hymn[]) => hymns
        .filter(hymn =>
        ...
        .sort((a : Hymn, b : Hymn) => hymnNumberFromHymn(a) < hymnNumberFromHymn(b) ? -1 : 1)
      )
    );
  }
}