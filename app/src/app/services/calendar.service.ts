...
import { HolyDay, Kalendar, Liturgy, LiturgicalDay, LiturgicalDocument, LiturgicalWeek, LiturgicalWeekIndex, ProperLiturgy, addOneDay, dateFromYMD, dateToYMD, liturgicalWeek, liturgicalDay, transferredFeast } from '@venite/ldf';
  ...
  /** Find `HolyDay`s connected to either a date or a slug */
   addHolyDays(day : LiturgicalDay, vigil : boolean) : Observable<LiturgicalDay> {
    ...
    const isNovember = date.getMonth() === 10, // January is 0, Feb 1, etc., so Sept is 8 
      ...
      thanksgiving$ = isNovember && isThursday && nthWeekOfMonth === 4 ? this.findSpecialDays(kalendar, 'thanksgiving-day') : of([] as HolyDay[]);
    // Transferred feasts
    const allHolyDays = KALENDAR[kalendar]
      ...
      transferred$ : Observable<HolyDay | null> = from(transferredFeast(
        async (dfd : Date) => {
          const week = await this.findWeek('bcp1979', liturgicalWeek(dfd)).toPromise();
    ...
    return combineLatest([feasts$, specials$, thanksgiving$, transferred$])
      .pipe(
        // OR together the feasts and specials
        map(([feasts, specials, thanksgiving, transferred]) => feasts
