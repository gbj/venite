...
export class OrganizationPage implements OnInit {
  ...
  ngOnInit() {
    this.orgId$ = this.route.params.pipe(map((params) => params.orgId));
    ...
    this.bulletins$ = this.orgId$.pipe(
      ...
      map((docs) =>
        docs.sort((a, b) =>
          dateFromYMDString(a?.day?.date) < dateFromYMDString(b?.day?.date)
            ? 1
            : -1
        )
      )
    ...
    this.role$ = combineLatest(
      this.auth.user,
      userProfile$.pipe(startWith(null)),
      this.orgId$,
      this.organization$
    ).pipe(
