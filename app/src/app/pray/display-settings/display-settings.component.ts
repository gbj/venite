...
export class DisplaySettingsComponent implements OnInit {
  ...
  voicesWithNationalities: {
    voice: SpeechSynthesisVoice;
    nationality: string;
  }[];
  fontAccessibility$: Observable<boolean>;
    ...
    if (this.hasVoice) {
      this.voicesWithNationalities = this.voiceChoices
        .map((voice) => {
          return {
            voice,
            nationality: this.translate.instant(`speech.${voice.lang}`),
          };
        })
  ...
  setFontAccessibility(ev: CustomEvent) {
    console.log("fontAccessibility ev = ", ev);
    this.preferencesService.set(
      "font_accessibility",
